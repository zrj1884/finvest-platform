...
import { getFundSnapshot, type FundNav } from '../api/market'
...
function toggleSort(field: string) {
  if (sortBy.value === field) {
    ...
    sortBy.value = field
    sortOrder.value = field === 'daily_return' || field === 'nav' ? 'desc' : 'asc'
  }
  page.value = 1
  loadData()
}
...
async function loadData() {
  loading.value = true
  try {
    const resp = await getFundSnapshot(
      page.value, pageSize.value,
      sortBy.value, sortOrder.value,
      searchQuery.value || undefined,
    )
    funds.value = resp.items
...
<template>
  <div>
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
      <h1 class="text-2xl font-bold text-gray-900">{{ t('market.fund') }}</h1>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th @click="toggleSort('nav')" class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase cursor-pointer hover:text-gray-700 select-none">
                  {{ t('table.nav') }} <span class="text-gray-400">{{ sortIcon('nav') }}</span>
                </th>
                <th @click="toggleSort('accumulated_nav')" class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase cursor-pointer hover:text-gray-700 select-none">
                  {{ t('table.accNav') }} <span class="text-gray-400">{{ sortIcon('accumulated_nav') }}</span>
                </th>
                <th @click="toggleSort('daily_return')" class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase cursor-pointer hover:text-gray-700 select-none">
                  {{ t('table.dailyReturn') }} <span class="text-gray-400">{{ sortIcon('daily_return') }}</span>
                </th>
