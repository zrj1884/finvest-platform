...
import { getBondSnapshot, type BondDaily } from '../api/market'
...
function toggleSort(field: string) {
  if (sortBy.value === field) {
    ...
    sortBy.value = field
    sortOrder.value = field === 'change_pct' || field === 'close' ? 'desc' : 'asc'
  }
  page.value = 1
  loadData()
}
...
async function loadData() {
  loading.value = true
  try {
    const resp = await getBondSnapshot(
      page.value, pageSize.value,
      sortBy.value, sortOrder.value,
      searchQuery.value || undefined,
    )
    bonds.value = resp.items
...
<template>
  <div>
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
      <h1 class="text-2xl font-bold text-gray-900">{{ t('market.bond') }}</h1>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-200">
              ...
              >
                ...
                <td class="px-4 py-3 text-sm text-right font-mono">{{ bond.close.toFixed(3) }}</td>
                <td
