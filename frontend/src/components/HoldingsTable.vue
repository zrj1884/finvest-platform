...
<template>
  <div class="bg-white rounded-lg shadow overflow-hidden">
    <div class="px-4 py-3 border-b border-gray-200">
      <h3 class="text-sm font-semibold text-gray-700">{{ t('portfolio.holdings') }}</h3>
    </div>
      ...
      <table class="min-w-full text-sm">
        <thead class="bg-gray-50">
          <tr>
            ...
            <th class="px-3 py-2 text-left text-xs font-medium text-gray-500">{{ t('common.market') }}</th>
            ...
            <th class="px-3 py-2 text-right text-xs font-medium text-gray-500">{{ t('position.avgCost') }}</th>
            <th class="px-3 py-2 text-right text-xs font-medium text-gray-500">{{ t('common.price') }}</th>
            <th class="px-3 py-2 text-right text-xs font-medium text-gray-500">{{ t('position.marketValue') }}</th>
            <th class="px-3 py-2 text-right text-xs font-medium text-gray-500">{{ t('position.pnl') }}</th>
            <th class="px-3 py-2 text-right text-xs font-medium text-gray-500">{{ t('portfolio.pnlPct') }}</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-100">
          <tr v-for="h in holdings" :key="`${h.account_id}-${h.symbol}`" class="hover:bg-gray-50">
            ...
            <td class="px-3 py-2 text-gray-500">{{ t(`market.${h.market}`) }}</td>
